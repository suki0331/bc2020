...
def create_model(optimizer='adam', drop=0.2):
    ...
    x = Conv2D(filters=64, kernel_size=3, activation='relu', name='hidden1')(input1)
    x = Dropout(rate=drop)(x)
    x = Conv2D(filters=64, kernel_size=3, activation='relu', name='hidden2')(x)
    x = Dropout(rate=drop)(x)
    x = Conv2D(filters=64, kernel_size=3, activation='relu', name='hidden3')(x)
    x = Dropout(rate=drop)(x)
    x = Flatten()(x)
