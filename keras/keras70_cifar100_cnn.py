...
layer1 = Conv2D(filters=256, kernel_size=5, padding='same', activation='relu')(input1) 
layer2 = Dropout(rate=0.2)(layer1)
layer3 = Conv2D(filters=256, kernel_size=3, padding='same', activation='relu')(layer2)
layer4 = Dropout(rate=0.3)(layer3)
layer5 = Conv2D(filters=512, kernel_size=3, padding='same', activation='relu')(layer4)
layer6 = Dropout(rate=0.4)(layer5)
layer7 = MaxPooling2D(pool_size=2)(layer6)
layer8 = Flatten()(layer7)
layer9 = Dense(1024, activation='relu')(layer8)
...
tb_hist = TensorBoard(log_dir='graph\keras70', histogram_freq=0,
                      write_graph=True, write_images=True)

hist = model.fit(x_train, y_train, batch_size=64, epochs=15, verbose=2, validation_split=0.1, callbacks=[early_stopping, model_checkpoint, tb_hist])
# evaluate
